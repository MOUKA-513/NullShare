    ...
    def start(self):
        ...
        if self.timeout:
            def stop_after_timeout():
                ...
                if self.running:
                    print(f"\n\033[93mTimeout ({self.timeout}s) reached. Server shutting down...\033[0m")
                    import os
                    os._exit(0)
...
if __name__ == "__main__":
    # Test the server
    print("Testing ShareServer class...")
    test_path = Path("test.txt")
    if not test_path.exists():
        test_path.write_text("Test content for NullShare")
    
    server = ShareServer(paths=[test_path], port=8080, timeout=5)
    print("Starting test server on port 8080 (will auto-stop in 5 seconds)...")
    server.start()
